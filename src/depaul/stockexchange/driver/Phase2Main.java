...
7) Publish Ticker for SBUX. Only user REX is subscribed, only REX gets a message (no arrows on message):
User REX Received Ticker for SBUX $52.00 ↑

8) Publish Ticker for IBM. Only user ANN is subscribed, only ANN gets a message (no arrows on message):
User ANN Received Ticker for IBM $204.85 ↑

9) Publish Ticker for GE. No user is subscribed, no user gets a message:

10) Publish Ticker for SBUX. Now all 4 users are subscribed so REX, ANN, OWL & BEN get a message (UP arrows on message):
...
11) Publish Ticker for SBUX. Now all 4 users are subscribed so REX, ANN, OWL & BEN get a message (DOWN arrows on message):
User REX Received Ticker for SBUX $51.90 ↓
User OWL Received Ticker for SBUX $51.90 ↓
User ANN Received Ticker for SBUX $51.90 ↓
User BEN Received Ticker for SBUX $51.90 ↓

12) Publish Ticker for SBUX. Now all 4 users are subscribed so REX, ANN, OWL & BEN get a message (EQUALS sign on message):
User REX Received Ticker for SBUX $51.90 =
User OWL Received Ticker for SBUX $51.90 =
User ANN Received Ticker for SBUX $51.90 =
User BEN Received Ticker for SBUX $51.90 =

13) Publish Ticker for SBUX. Now all 4 users are unsubscribed so no one get a message
Done with Ticker tests

14) Publish Last Sale for SBUX. Only user REX is subscribed, only REX gets a message (Last Sale & Ticker with DOWN arrow):
User REX Received Last Sale for SBUX 120@$51.00

15) Publish Last Sale for IBM. Only user ANN is subscribed, only ANN gets a message (Last Sale & Ticker with UP arrow):
User ANN Received Last Sale for IBM 300@$205.85

16) Publish Last Sale for GE. No user is subscribed, no user gets a message:

17) Publish Last Sale for SBUX. Now all 4 users are subscribed so REX, ANN, OWL & BEN get a message (Last Sale & Ticker with EQUALS sign):
User REX Received Last Sale for SBUX 120@$51.00
...
18) Publish Last Sale for SBUX. Now all 4 users are unsubscribed so no one get a message
Done with Last Sale tests

19) Send a CancelMessage to REX. REX is subscribed for SBUX messages so REX gets the message.
...
20) Send a CancelMessage to REX. REX is NOT subscribed for IBM messages so REX does not get the message.

21) Send a CancelMessage to ANN. ANN is subscribed for SBUX messages so ANN gets the message.
...
22) Send a CancelMessage to ANN. ANN is NOT subscribed for IBM messages so ANN does not get the message.

23) Send a FillMessage to REX. REX is subscribed for SBUX messages so REX gets the message.
User REX Received Fill Message: User: REX, Product: SBUX, Price: $52.00, Volume: 140, Details: Cancelled By User, Side: BUY, Id: ABC123XYZ

24) Send a FillMessage to REX. REX is NOT subscribed for IBM messages so REX does not get the message.

25) Send a FillMessage to ANN. ANN is subscribed for SBUX messages so ANN gets the message.
User ANN Received Fill Message: User: ANN, Product: SBUX, Price: $52.00, Volume: 140, Details: Cancelled By User, Side: BUY, Id: ABC123XYZ

26) Send a FillMessage to ANN. ANN is NOT subscribed for IBM messages so ANN does not get the message.

27) Send a MarketMessage. REX is the only MessagePublisher subscriber so only REX gets the message.
User REX Received Market Message: depaul.stockexchange.message.MarketMessage@6bc7c054

28) Send a MarketMessage. REX and ANN are MessagePublisher subscribers so REX & Ann get the message.
User REX Received Market Message: depaul.stockexchange.message.MarketMessage@232204a1
User ANN Received Market Message: depaul.stockexchange.message.MarketMessage@232204a1

29) Send a MarketMessage. No users are MessagePublisher subscribers so No users get the message.
