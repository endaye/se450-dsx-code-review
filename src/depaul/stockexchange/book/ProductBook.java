 ...
 * 
 * the list of not-yet-traded (i.e., “booked”) Orders and 
 * Quote Sides (Tradables) for the Buy side and for the Sell side. 
 * To represent this, the ProductBook will own two ProductBookSide objects 
 * – one for the BUY side and one for the SELL side.
 * 
 * @author jimliu
 */
public class ProductBook {
    ...
    /**
     * The String stock symbol that this book represents (i.e., MSFT, IBM, AAPL, etc).
     */
     ...
     * Set the product
     * @param product
     *      The String stock symbol that this book represents 
     *      (i.e., MSFT, IBM, AAPL, etc).
     * @throws DataValidationException 
     *      If product is empty or null
     */
    ...
    /**
     * A String that will hold the toString results of the latest 
     * Market Data values (the prices and the volumes at the top of 
     ...
     * 
     * @param product
     *      The String stock symbol this book represents
     * @throws DataValidationException
    ...
    /**
     * This method should return an ArrayList containing any orders 
     * for the specified user that have remaining quantity.
     * @param userName
     *      The specified user 
     * @return
    ...
    void publishCancel(Tradable trade, String details) 
            throws DataValidationException, DataValidationException, InvalidMessageDataException {
    ...
    public synchronized String[][] getBookDepth() {
        ...
        bd[1] = this.buySide.getBookDepth();
     ...
     * @return 
     *      A MarketDataDTO containing the best buy/side side price and volume
     */
    ...
    /**
     * This method should add the Tradable passed in 
     * to the “oldEntries” HashMap.
     * @param t 
     *      The tradable
     * @throws DataValidationException
     *      If the tradable is null
     */
     ...
     */
    public synchronized void openMarket() throws NotSubscribedException, DataValidationException, InvalidPriceOperation, InvalidMessageDataException {
        Price buyPrice = this.buySide.topOfBookPrice();
        Price sellPrice = this.sellSide.topOfBookPrice();
        ...
        while (buyPrice.greaterOrEqual(buyPrice) || buyPrice.isMarket()
                        || sellPrice.isMarket()) {
    ...
    /**
     * This method will cancel the Order specified by the provided orderId 
    ...
    /**
     * This method will cancel the specified user’s Quote 
     * on the both the BUY and SELL sides. 
     * @param userName
    ...
    /**
     * This method should add the provided Quote’s sides to 
     * the Buy and Sell ProductSideBooks.
     * @param q 
    ...
    public synchronized void addToBook(Quote q) 
            throws DataValidationException, DataValidationException, NotSubscribedException, InvalidTradableValue, InvalidPriceOperation, InvalidMessageDataException {
        if (q == null) {
    ...
    /**
     * This method should add the provided Order to 
     * the appropriate ProductSideBook. 
     * @param o 
     *      The provided Order
     * @throws DataValidationException 
     * @throws InvalidMessageDataException 
    ...
    /**
     * This method needs to determine if the “market” for this stock product 
     * has been updated by some market action.
     */
    public synchronized void updateCurrentMarket() {
        String s = String.format("%s%s%s%s", this.buySide.topOfBookPrice(),
                this.buySide.topOfBookVolume(), this.sellSide.topOfBookPrice(),
                this.sellSide.topOfBookVolume());
        
        if (!s.equals(lastCurrentMarket)) {
            MarketDataDTO dto = new MarketDataDTO(this.product, this.buySide.topOfBookPrice(),
                this.buySide.topOfBookVolume(), this.sellSide.topOfBookPrice(),
                this.sellSide.topOfBookVolume());
            try {
    ...
    /**
     * This method deals with the addition of Tradables to 
     * the Buy/Sell ProductSideBook and handles the results of 
     * any trades the result from that addition. 
     * @param side
     *      The Buy/Sell side
     * @param trd
     *      The tradable object
     * @throws DataValidationException 
     *      The tradable is null
     * @throws InvalidMessageDataException 
     * @throws InvalidPriceOperation 
     */
    private synchronized void addToBook(BookSide side, Tradable trd)
        ...
        if (allFills != null && !allFills.isEmpty()) {
            ...
            LastSalePublisher.getInstance()
                    .publishLastSale(trd.getProduct(), lastSalePrice, traded);
        }
        
        if (trd.getRemainingVolume() > 0) {
            if (trd.getPrice().isMarket()) {
                // @Todo: use trd's id as cm's id?
                CancelMessage cm = new CancelMessage(trd.getUser()
                        , trd.getProduct(), trd.getPrice(), trd.getRemainingVolume()
                        , "Cancelled", side, trd.getId());
                MessagePublisher.getInstance().publishCancel(cm);
            } else {
                bookSide.addToBook(trd);
            }
