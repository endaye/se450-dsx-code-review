...
/**
 * Owns a list of the “booked” Tradable objects, organized by Price 
 * and by time of arrival.
 * An individual ProductBookSide represents the Buy side or the Sell side. 
 * Each ProductBookSide also owns a TradeProcessor object.
 * 
 * @author jimliu
 */
public class ProductBookSide {
    ...
    /**
     * A collection of book entries for this side, represented in a way 
     * that organizes the book entries by Price, and then by time of arrival. 
     */
    ...
    /**
     * A reference back to the ProductBook object 
     * that this ProductBookSide belongs to.
     */
    private ProductBook parent;

	private Tradable tradableFound;
     ...
     *      If the ProductBook object is null
     */
     ...
     * 
     * @param book
     *      A reference to the ProductBook object 
     *      that this ProductBookSide belongs to
     * @param side 
     *      A side indicator (BUY or SELL) that specifies the “side” 
     *      that this ProductBookSide represents
     * @throws depaul.stockexchange.book.DataValidationException
     *      If the book is null
     */
     ...
     * containing information on all the orders in this ProductBookSide 
     * that have remaining quantity for the specified user.
     * @param userName
     * @return 
     */
        ...
        if (orders != null) {
            ...
            for (Tradable order : orders) {
                if (order.getRemainingVolume() > 0) {
                    TradableDTO td = new TradableDTO(order);
                    tradables.add(td);
                }
            }
        }
    ...
    /**
     * This method should return an ArrayList of the Tradables that are at the best price in the “bookEntries” HashMap.
     * @return 
     *      Get the first Price in that ArrayList
     */
     ...
     * where each index holds a “Price x Volume” String. 
     * 
     * @return 
     *      An array of Strings
     */
    ...
    /**
     * This method should return all the Tradables in this book side 
...
This method should return true if the product book (the “bookEntries” HashMap) contains a Market Price 
    ...
    */
     ...
     * @return 
     *      if a MarketPrice is a key in the “bookEntries” HashMap 
     *      you return true, otherwise false
     */
    ...
    /**
     * This method should return true if the ONLY Price 
     * in this product’s book is a Market Price.
     * @return 
     *      if there is only one key in the “bookEntries” HashMap and it is 
     *      a Market Price, you return true, otherwise false
     */
    public synchronized boolean hasOnlyMarketPrice() {
        return bookEntries.size() == 1 
                && bookEntries.keySet().iterator().next().isMarket();
    }
     ...
     *      Otherwise, create a sorted ArrayList of Prices. 
     *      Then return the first Price in that list.    
     */
     ...
     * @return 
     *      If the “bookEntries” HashMap is empty, then return zero.
...
Returns true if the product book (the “bookEntries” HashMap) is empty, false otherwise.
    */
     ...
     * @return 
     *      If the product book (the “bookEntries” HashMap) is empty, return true,
     *      Otherwise false
     */
    ...
    /**
     * Get all the Tradables in the “bookEntries” HashMap.
     * @return 
     *      ArrayList of the Tradables in the “bookEntries” HashMap.
     */
    ...
    public synchronized void cancelAll() 
            throws DataValidationException, OrderNotFoundException, InvalidMessageDataException {
     ...
     * Note, if the Quote was the last Tradable in the ArrayList of Tradables 
     * at that price, remove the price entry from the “bookEntries” HashMap
     * @param user
     *      The specified user
     * @return 
     *      Return the DTO from the method
     */
    public synchronized TradableDTO removeQuote(String user) 
        ...
        for (Price price : bookEntries.keySet()) {
            ...
            if (tradableFound != null && tradables.isEmpty()) {
                ...
                break; // should break, otherwise can't go on with the for loop.
            }
        }
    ...
    /**
     * This method should cancel the Order (if possible) 
    ...
    /**
     * This method should cancel the QuoteSide (if possible) 
     * that has the specified userName. 
     * @param userName
    ...
    public synchronized void submitQuoteCancel(String userName)
            throws DataValidationException, InvalidMessageDataException {
        if (Utils.isNullOrEmpty(userName)) {
            throw new DataValidationException("userName can't be null or empty.");
        }
    ...
    /**
     * This method should add the Tradable passed in to 
     * the “parent” product book’s “old entries” list. 
     * @param t 
     *      The Tradable to be added
     * @throws DataValidationException
     *      If t is null
     */
    ...
    public synchronized void addToBook(Tradable trd)
        ...
        if (bookEntries == null) { // can't find by price, add it
    ...
    /**
     * This method will attempt a trade the provided 
     * Tradable against entries in this ProductBookSide. 
     * @param trd
     *      the provided Tradable
     * @return 
     * @throws depaul.stockexchange.book.DataValidationException 
     *      If tradable is null
    ...
    public HashMap<String,FillMessage> tryTrade(Tradable trd)
            throws DataValidationException, NotSubscribedException, InvalidPriceOperation, InvalidMessageDataException {
        if (trd == null) {
            throw new DataValidationException("The tradable can't be null.");
        }
        
        // If side is null
        if (trd.getSide() != BookSide.BUY && trd.getSide() != BookSide.SELL) {
    ...
    /**
     * This method will try to fill the SELL side Tradable 
     * passed in against the content of the book. 
     * @param trd
     *      The SELL side Tradable
     * @return
     *      
     * @throws DataValidationException 
     * @throws InvalidPriceOperation 
    ...
    public HashMap<String,FillMessage> trySellAgainstBuySideTrade(Tradable trd)
            throws DataValidationException, InvalidPriceOperation, InvalidMessageDataException {
        if (trd == null) {
            throw new DataValidationException("The tradable can't be null.");
        }
    ...
    /**
     * This method is designed to merge multiple fill messages 
     * together into one consistent list. 
     * When one user’s Order of QuoteSide trades against multiple Tradable 
     * objects at the same price – only ONE fill should be sent 
     * (for the total traded volume). 
     * @param existing
     *      The existing Order of QuoteSide trades
     * @param newOnes
     *      The new multiple Tradable objects
     * @return 
     *      The results after merged
     * @throws InvalidMessageDataException 
     */
    private HashMap<String, FillMessage> mergeFills(
            HashMap<String,FillMessage> existing, 
            HashMap<String, FillMessage> newOnes) throws DataValidationException, InvalidMessageDataException {
        if (existing == null) {
            throw new DataValidationException("The tradable can't be null.");
        }
    ...
    /**
     * This method will try to fill the BUY side Tradable passed in 
     * against the content of the book. 
     * @param trd
     *      The BUY side Tradable
     * @return 
     * @throws InvalidMessageDataException 
     * @throws InvalidPriceOperation 
     */
    public synchronized HashMap<String, FillMessage> 
        tryBuyAgainstSellSideTrade(Tradable trd) throws DataValidationException, InvalidPriceOperation, InvalidMessageDataException {
        if (trd == null) {
            throw new DataValidationException("The tradable can't be null.");
        }
    ...
    /**
     * This method will remove an key/value pair from the book
